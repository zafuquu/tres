...
export const storage = {
  ...
  async get(key) {
    ...
    try {
      ...
      // Remote is the shared source of truth (e.g. corrections made directly
      // in Supabase or via re-seeding). Passing it second here means it wins
      // over a stale local cache when both have a complete slot for the same
      // date/period — otherwise a browser that already cached an old value
      // would keep re-asserting it forever, even after the DB is fixed.
