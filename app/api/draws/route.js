...
import { sql, ensureSchema, rowToRecord } from "../../../lib/db";

export const dynamic = "force-dynamic";

// The frontend (chatgpt_app_fixed / any other client) is deployed on a
// different domain than this backend, so every request is cross-origin.
// Without these headers the browser silently blocks the response before
// the app ever sees it — which is exactly why sync looked broken.
const CORS_HEADERS = {
...
export async function GET() {
  try {
    await ensureSchema();
    const { rows } = await sql`SELECT * FROM draws ORDER BY draw_date ASC;`;
...
export async function POST(req) {
  try {
    ...
    await sql`
      ...
      VALUES (${date}, ${slot1[0]}, ${slot1[1]}, ${slot1[2]}, ${slot2[0]}, ${slot2[1]}, ${slot2[2]}, ${slot3[0]}, ${slot3[1]}, ${slot3[2]}, 'manual')
